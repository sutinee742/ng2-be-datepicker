...
export class MyDatePicker implements OnChanges, ControlValueAccessor {
    ...
    thSelectionDayTxt: string = ""; //editsourcecode
    beYearSelected: number; //editsourcecode
    ...
    onUserDateInput(value: string): void {
        ...
        console.log('4: input')
        var be = value;
        var ce = this.beToce(be);

        console.log('BC: '+be+' to AC: '+ce);
        ...
        if (value.length === 0) {
            ...
            let date: IMyDate = this.utilService.isDateValid(ce, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableDays, this.opts.disableDateRanges, this.opts.monthLabels, this.opts.enableDays);
        ...
        if (this.invalidDate) {
            this.inputFieldChanged.emit({value: ce, dateFormat: this.opts.dateFormat, valid: !(value.length === 0 || this.invalidDate)});
            this.onChangeCb("");
            this.onTouchedCb();
        }
    ...
    onUserYearInput(value: string): void {
        //editsourcecode
        this.invalidYear = false;
        let y: number = this.utilService.isYearLabelValid(Number(value)-543, this.opts.minYear, this.opts.maxYear);
        if (y !== -1) {
            this.editYear = false;
    ...
    updateDateValue(date: IMyDate, clear: boolean): void {
        ...
        console.log('2: '+ this.selectionDayTxt);

        if(this.selectionDayTxt === ''){
            this.thSelectionDayTxt = '';
            
        }else{
            var ce = this.selectionDayTxt;
            var be = this.ceTobe(ce);

            console.log('CE: '+ce+' to BE: '+be);

            this.thSelectionDayTxt = be;
        }
    ...
    generateCalendar(m: number, y: number, notifyChange: boolean): void {
        ...
        this.beYearSelected = this.visibleMonth.year + 543;  //editsourcecode
    ...
    parseSelectedDate(selDate: any): IMyDate {
        ...
        console.log('3: '+ this.selectionDayTxt);
    ...
    //editsourcecode
    ceTobe(ce: string): string {
        var tmpdate = ce.split("-");
        var tmpyear = parseInt(tmpdate[2]) + 543;   
        var be = tmpdate[0]+'-'+tmpdate[1]+'-'+tmpyear;   
        return be;
    }

    //editsourcecode
    beToce(be: string): string {
        var tmpdate = be.split("-");
        var tmpyear = parseInt(tmpdate[2]) - 543; 
        var ce = tmpdate[0]+'-'+tmpdate[1]+'-'+tmpyear;  
        return ce;
    }
}
